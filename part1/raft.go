...
func (cm *ConsensusModule) RequestVote(args RequestVoteArgs, reply *RequestVoteReply) error {
	...
	cm.dlog("收到 RequestVote 请求: %+v [currentTerm=%d, votedFor=%d]", args, cm.currentTerm, cm.votedFor)

	if args.Term > cm.currentTerm {
		cm.dlog("... 当前任期早于 Request 任期，变为 Follower")
...
func (cm *ConsensusModule) AppendEntries(args AppendEntriesArgs, reply *AppendEntriesReply) error {
	...
	cm.dlog("收到 AppendEntries 请求：%+v", args)
...
func (cm *ConsensusModule) runElectionTimer() {
	...
	for {
		...
		if termStarted != cm.currentTerm {
			cm.dlog("选举定时器中的 term 从 %d 变为 %d，退出", termStarted, cm.currentTerm)
			cm.mu.Unlock()
			return
		}
...
func (cm *ConsensusModule) startElection() {
	...
	for _, peerId := range cm.peerIds {
		go func(peerId int) {
			...
			cm.dlog("向服务器 %d 发送 RequestVote 请求：%+v", peerId, args)
			if err := cm.server.Call(peerId, "ConsensusModule.RequestVote", args, &reply); err == nil {
				cm.mu.Lock()
				defer cm.mu.Unlock()

				cm.dlog("收到 RequestVote 应答：%+v", reply)

				if cm.state != Canditate {
					cm.dlog("等待 RequestVote 回复时，状态变为 %v", cm.state)
					return
				}

				if reply.Term > savedCurrentTerm {
					cm.dlog("当前任期 term 早于 RequestVote 应答中的任期")
...
func (cm *ConsensusModule) startLeader() {
	cm.state = Leader
	cm.dlog("成为 Leader；term=%d，log=%v", cm.currentTerm, cm.log)
	...
	for _, peerId := range cm.peerIds {
		...
		go func(peerId int) {
			...
			if err := cm.server.Call(peerId, "ConsensusModule.AppendEntries", args, &reply); err == nil {
				...
				if reply.Term > savedCurrentTerm {
					cm.dlog("当前任期 term 早于 heartbeat 应答中的任期")
					cm.becomeFollower(reply.Term)
					return
				}
			}
